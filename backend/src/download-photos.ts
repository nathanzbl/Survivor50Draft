...
const PHOTO_DIR = path.resolve(__dirname, '../../frontend/public/cast-photos');
...
function nameToFilename(name: string): string {
  return name.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-') + '.webp';
}
...
async function downloadPhotos() {
  ...
  try {
    let updated = 0;
    for (const name of Object.keys(photoUrls)) {
      const filename = nameToFilename(name);
      const localUrl = `/cast-photos/${filename}`;
    ...
    console.log(`Updated ${updated} player photo URLs to local paths`);
  } finally {
    client.release();
    await pool.end();
  }
